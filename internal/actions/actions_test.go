...
func TestBuildPopupAction(t *testing.T) {
	...
	want := []string{"run-shell", "-b", "sleep 0.05; 'tmux' 'display-popup' '-E' '-d' '#{pane_current_path}' '-s' 'fg=#ffffff,bg=#2d2b55' '-S' 'fg=#fad000' '-w' '90%' '-h' '50%' 'lazygit'"}
...
func TestBuildPopupActionUsesCommandSizeOverride(t *testing.T) {
	...
	want := []string{"run-shell", "-b", "sleep 0.05; 'tmux' 'display-popup' '-E' '-d' '#{pane_current_path}' '-s' 'fg=#ffffff,bg=#2d2b55' '-S' 'fg=#fad000' '-w' '95%' '-h' '85%' 'lazygit'"}
	if strings.Join(action.Args, "\x00") != strings.Join(want, "\x00") {
		t.Fatalf("args = %#v, want %#v", action.Args, want)
	}
}
...
func TestBuildPopupActionCanOverrideOnlyOneSize(t *testing.T) {
	...
	want := []string{"run-shell", "-b", "sleep 0.05; 'tmux' 'display-popup' '-E' '-d' '#{pane_current_path}' '-s' 'fg=#ffffff,bg=#2d2b55' '-S' 'fg=#fad000' '-w' '90%' '-h' '85%' 'lazygit'"}
	if strings.Join(action.Args, "\x00") != strings.Join(want, "\x00") {
		t.Fatalf("args = %#v, want %#v", action.Args, want)
	}
}
