...
func TestPopupArgs(t *testing.T) {
	...
	want := []string{"display-popup", "-E", "-s", "fg=#ffffff,bg=#2d2b55", "-S", "fg=#ff9d00", "-w", "75%", "-h", "70%", "tmux-commander"}
	if len(got) != len(want) {
...
func TestPopupArgsWithoutBorder(t *testing.T) {
	...
	want := []string{"display-popup", "-E", "-B", "-s", "fg=#ffffff,bg=#2d2b55", "-S", "fg=#ff9d00", "-w", "75%", "-h", "70%", "tmux-commander"}
	if len(got) != len(want) {
		t.Fatalf("arg count = %d, want %d: %#v", len(got), len(want), got)
	}
	for i := range want {
